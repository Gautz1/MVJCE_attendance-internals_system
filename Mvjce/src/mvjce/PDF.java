...
"from Subject_names as c join Student_info as s on c.Course_code=s.Course_code where s.USN='"+info.getString("USN")+"'");
...
"from Scheme as c join Student_info as s on c.Course_code=s.Course_code where s.USN='"+info.getString("USN")+"'");
            info1.next();for(int i=0;i<8;i++){subcode[i]=info1.getString(i+1);}
            info1=st1.executeQuery("select s1.sub1_int1,s1.sub2_int1,s1.sub3_int1,s1.sub4_int1,s2.sub5_int1,s2.sub6_int1,s2.sub7_int1,s2.sub8_int1 " +
"from internals as s1 join internals2 as s2 on s1.USN=s2.USN where s1.USN='"+info.getString("USN")+"'");
...
"from attendance where USN='"+info.getString("USN")+"'");
            info1.next();for(int i=0;i<8;i++){intr[i]=info1.getString(i+1);}
            create(j);j++;
        }
	...
	OutputStream outputStream = 
		new FileOutputStream(new File("/home/narein/NetBeansProjects/pdffolder/testpdf"+sino+".pdf"));
...
"from attendance where USN='1MJ10AE002';");*/
        ...
        cell.setRowspan(3);cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("I Internal Test Marks",new Font(FontFamily.TIMES_ROMAN, 9,Font.BOLD)));
        cell.setRowspan(3);cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        table.addCell(cell);
